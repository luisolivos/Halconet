...
namespace Ventas.Ventas.ScoreCard
{
    public partial class frmWP : Form
    {
        ...
        {
            SlpCode, Vendedor, Sucursal, PZ, TendenciaPZ, Venta, TendenciaFinMes, Bono, Boton
        ...
        public frmWP(int rolUsuario, int codigoVendedor, string nombreUsuario, string sucursal)
        {
        ...
        private void DarFormatoGrid()
        {
            ...
            gridLineas.Columns[(int)ColumnasGrid.PZ].Width = 90;
            gridLineas.Columns[(int)ColumnasGrid.TendenciaPZ].Width = 90;
            gridLineas.Columns[(int)ColumnasGrid.Venta].Width = 90;
            gridLineas.Columns[(int)ColumnasGrid.TendenciaFinMes].Width = 90;
            gridLineas.Columns[(int)ColumnasGrid.Bono].Width = 90;

            gridLineas.Columns[(int)ColumnasGrid.PZ].DefaultCellStyle.Format = "N0";
            gridLineas.Columns[(int)ColumnasGrid.TendenciaPZ].DefaultCellStyle.Format = "N0";
            ...
            gridLineas.Columns[(int)ColumnasGrid.TendenciaPZ].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            gridLineas.Columns[(int)ColumnasGrid.PZ].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
        ...
        private void btnPresentar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                gridTotales.DataSource = null;
                gridLineas.DataSource = null;
                SqlCommand commandLienasMoradas = new SqlCommand("PJ_ScoreCardVentas", new SqlConnection(ClasesSGUV.Propiedades.conectionSGUV));
                commandLienasMoradas.CommandType = CommandType.StoredProcedure;
                commandLienasMoradas.Parameters.AddWithValue("@TipoConsulta", 8);
                ...
                decimal VentaPZ = 0;
                decimal PronosticoFinMesPZ = 0;
                decimal Bono = 0;

                foreach (DataGridViewRow item in gridLineas.Rows)
                {
                    VentaPZ += Convert.ToDecimal(item.Cells[(int)ColumnasGrid.PZ].Value);
                    PronosticoFinMesPZ += Convert.ToDecimal(item.Cells[(int)ColumnasGrid.TendenciaPZ].Value);
                ...
                tblTotales.Columns.Add("Venta (PZ)");
                tblTotales.Columns.Add("Tendencia fin mes (PZ)");
                ...
                registro["Venta (PZ)"] = VentaPZ.ToString("N0");
                registro["Tendencia fin mes (PZ)"] = PronosticoFinMesPZ.ToString("N0");
