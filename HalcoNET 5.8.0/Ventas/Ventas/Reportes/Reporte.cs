...
namespace Ventas
{
    public partial class Reporte : Form
    {
        ...
        public Reporte(int rolUsuario, int codigoVendedor, string nombreUsuario, string sucursal)
        {
            InitializeComponent();
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataRowView item in clbLinea.CheckedItems)
                {
                    if (item["Codigo"].ToString() != "0")
                    {
                        if (!clbLinea.ToString().Equals(string.Empty))
                        {
                            stbLineas.Append(",");
                        }
                        stbLineas.Append(item["Codigo"].ToString());
                    }
                }
                if (clbLinea.CheckedItems.Count == 0)
                {
                    foreach (DataRowView item in clbLinea.Items)
                    {
                        if (item["Codigo"].ToString() != "0")
                        {
                            if (!clbLinea.ToString().Equals(string.Empty))
                ...
                StringBuilder stbGranCanal = new StringBuilder();
                foreach (DataRowView item in clbGranCanal.CheckedItems)
                {
                    if (item["Codigo"].ToString() != "0")
                    {
                        if (!clbGranCanal.ToString().Equals(string.Empty))
                        {
                            stbGranCanal.Append(",");
                        }
                        stbGranCanal.Append(item["Codigo"].ToString());
                    }
                }
                if (clbGranCanal.CheckedItems.Count == 0)
                {
                    foreach (DataRowView item in clbGranCanal.Items)
                    {
                        if (item["Codigo"].ToString() != "0")
                        {
                            if (!clbGranCanal.ToString().Equals(string.Empty))
                ...
                StringBuilder stbCanal = new StringBuilder();
                foreach (DataRowView item in clbCanal.CheckedItems)
                {
                    if (item["Codigo"].ToString() != "0")
                    {
                        if (!clbCanal.ToString().Equals(string.Empty))
                        {
                            stbCanal.Append(",");
                        }
                        stbCanal.Append(item["Codigo"].ToString());
                    }
                }
                if (clbCanal.CheckedItems.Count == 0)
                {
                    foreach (DataRowView item in clbCanal.Items)
                    {
                        if (item["Codigo"].ToString() != "0")
                        {
                            if (!clbCanal.ToString().Equals(string.Empty))
                ...
                GranCanales = stbGranCanal.ToString();
                Canales = stbCanal.ToString();
                ...
                command.Parameters.AddWithValue("@TipoConsulta", (int)Constantes.TipoConsultaPJ.ConsultaReporte);
                ...
                command.Parameters.AddWithValue("@GranCanales", GranCanales);
                command.Parameters.AddWithValue("@Canales", Canales);
                ...
                foreach (DataRow row in table.Rows)
                {
                    ...
                    totalPrecioCPP += row.Field<decimal>("Precio CPP Unitario");
                    totalPrecioPorVolumen += row.Field<decimal>("SubTotal Precio Volumen");
                ...
                tabla.Columns.Add("Precio Compra");
                tabla.Columns.Add("Precio Venta");
                tabla.Columns.Add("Precio CPP");
                tabla.Columns.Add("Precio Por Volumen");
                tabla.Columns.Add("Utilidad");
                DataRow registro = tabla.NewRow();
                registro["Precio Compra"] = totalPrecioCompra.ToString("c");
                registro["Precio Venta"] = totalPrecioVenta.ToString("c");
                registro["Precio CPP"] = totalPrecioCPP.ToString("c");
                registro["Precio Por Volumen"] = totalPrecioPorVolumen.ToString("c");
        ...
        private void btnExportar_Click(object sender, EventArgs e)
        {
            ...
            if (excel.Exportar(gridDetalles, false) == true)
            {
        ...
        private void DarFormatoGridPrincipal()
        {
            ...
            gridReporte.Columns[(int)ColumnasGrid.PrecioCPP].Width = 90;
            gridReporte.Columns[(int)ColumnasGrid.PrecioCPP].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
            gridReporte.Columns[(int)ColumnasGrid.PrecioCPP].DefaultCellStyle.Format = "C2";
