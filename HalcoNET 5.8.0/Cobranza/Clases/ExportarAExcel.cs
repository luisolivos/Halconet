    ...
    public class ExportarAExcel
    {
        ...
        public bool Exportar(DataGridView grd, bool eliminarUltimo)
        {
            ...
            {//DataGridView1.Columns.RemoveAt(1)
                ...
                fichero.Filter = "Excel (*.xls)|*.xls|Archivo de texto delimitado por tabulacones(*.txt)|*.txt";
                if (fichero.ShowDialog() == DialogResult.OK)
                {
                    if (Path.GetExtension(fichero.FileName.ToLower()).Equals(".xls"))
                    {
                    ...
                    {
                        return ExportarSinFormato(grd, fichero.FileName);
                    }
                }
        ...
        public bool ExportarSinFormato(DataGridView grd, string path)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(path))
                {
                    ...
                    File.WriteAllLines(path, encabezado, Encoding.UTF8);
        ...
        //                for (int i = 0; i < grd.Columns.Count; i++)
        //                {
        //                    file.Write(row.Cells[i].Value.ToString().Replace(',', ' ') + "\t");
        //                }
        //                file.Write("\n");
        //            }
